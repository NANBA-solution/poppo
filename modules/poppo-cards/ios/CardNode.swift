...
final class CardNode: SCNNode {
  ...
  private func applyMaterial(face: CardFaceData) {
    ...
    material.shininess = compactRender ? 0.62 : 0.9
  ...
  private func applyRaritySettings(face: CardFaceData) {
    let theme = CardFaceTheme.theme(for: face)
    let holoScale: Float = compactRender ? 0.58 : 1.0
    uniforms.holoIntensity = theme.holoIntensity * holoScale
    uniforms.baseTint = theme.holoTint
    uniforms.fresnelPower = compactRender ? rarity.fresnelPower + 0.35 : rarity.fresnelPower
    let shine: CGFloat = compactRender ? 0.62 : 0.9
    uniforms.shininess = Float(shine)
    holoMaterial?.shininess = shine
